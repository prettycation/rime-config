...
local tk = {
  default             = safe_require("nekocat.layouts.45keys"),
  ...
  wanxiang_zrm        = safe_require("nekocat.layouts.45keys_hint_zrm"),
  ...
  wanxiang_english    = merge(safe_require("nekocat.layouts.45keys"),
    keyboard { name = "万象英文" }),
  luna_pinyin         = merge(safe_require("nekocat.layouts.45keys"),
    keyboard { name = "朙月拼音", label_transform = "UPPERCASE" }),
  luna_pinyin_simp    = merge(safe_require("nekocat.layouts.45keys"),
    keyboard { name = "明月拼音", label_transform = "UPPERCASE" }),
  triple_pinyin_lssp  = safe_require("nekocat.layouts.38keys_hint"),
  wanxiang_yoemin     = safe_require("nekocat.layouts.38keys_hint"),
  english             = merge(safe_require("nekocat.layouts.45keys"),
